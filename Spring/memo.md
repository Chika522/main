...
### バインド：画面の入力内容とJavaのクラスをマッピングすること、バインドするJavaクラスはModelに登録する  
* バインドすることで画面の入力内容をサーバー側のコントローラークラスで受け取ることができる
* 画面ではth:fieldを使用してJavaクラスのフィールドをマッピングする
...
---  
## ✏️7章　画面レイアウト  
🌟画面作成にはレイアウト機能を使うのが一般的、各画面で共通のHTMLを毎回書く必要がなくなる  
  
### layout:replaceとlayout:fragmentの違い  
* `layout:replace`: タグを丸ごと入れ替える
* `layout:fragment`: タグ内にHTMLを追加する  
  
---
## ✏️8章　Mybatis  
#### 🌸**Mybatis**: 高度な機能が用意されたO/Rマッパー（Javaクラス（オブジェクト）とDBをマッピングするフレームワーク）　　
※ O/R = Object / Relation(テーブル)  
